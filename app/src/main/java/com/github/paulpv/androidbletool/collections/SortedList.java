...
/**
 * A variation of {@link androidx.recyclerview.widget.SortedList}.
 * <p>
 * Will eventually:
 * 1) Provide public method to manually re-sort.
 * 2) Support secondary/tertiary/etc sorts.
 */
...
public class SortedList<T> {
    ...
    private void addAllInternal(T[] newItems) {
        ...
        final int newSize = sortAndDedup(newItems, newItems.length);
    ...
    private void replaceAllInternal(@NonNull T[] newData) {
        ...
        mNewDataStart = 0;
        int newSize = sortAndDedup(newData, newData.length);
        ...
        while (mNewDataStart < newSize || mOldDataStart < mOldDataSize) {
            ...
            int result = compare(oldItem, newItem);
            if (result < 0) {
                replaceAllRemove();
            } else if (result > 0) {
    ...
    private String toString(T[] items) {
        ...
        for (int i = 0; i < length; ++i) {
            sb.append('\n').append(i).append(' ').append(items[i]).append(", ");
        }
        if (length > 0) {
            sb.append('\n');
        }
    ...
    private void sort(@NonNull T[] items, int length) {
        ...
        if (DEBUG_LOG) {
            ...
            Log.e(TAG, "sort: Arrays.sort(items, 0, length - 1, comparator)");
        }
    ...
    private int compare(T o1, T o2) {
        ...
        if (DEBUG_LOG) {
            ...
            Log.e(TAG, "compare: o1=" + o1);
            Log.e(TAG, "compare: o2=" + o2);
        }
     ...
     *
     * @return Number of de-duplicated items at the beginning of the array.
     */
    private int sortAndDedup(@NonNull T[] items, int length) {
        if (length == 0) {
            return 0;
        }
        ...
        for (int i = 1; i < length; ++i) {
            ...
            int compare = compare(items[rangeStart], currentItem);
    ...
    private void merge(T[] newData, int newDataSize) {
        ...
        while (mOldDataStart < mOldDataSize || newDataStart < newDataSize) {
            ...
            int compare = compare(oldItem, newItem);
            if (compare > 0) {
    ...
    /**
     * Called by {@link #add(Object)}, {@link #recalculatePositionOfItemAt(int)}, and {@link #updateItemAt(int, Object)}
     *
     * @param item
     * @param notify
     * @return
    ...
    private int add(T item, boolean notify) {
        if (DEBUG_LOG) {
            Log.e(TAG, "add(item=" + item + ", notify=" + notify + ")");
            Log.e(TAG, "add: BEFORE mData=" + Arrays.toString(mData));
        }
    ...
    public void updateItemAt(int index, T item) {
        ...
        if (existing != item) {
            ...
            final int cmp = compare(existing, item);
            if (cmp == 0) {
                mData[index] = item;
                if (contentsChanged) {
                    mCallback.onChanged(index, 1, mCallback.getChangePayload(existing, item));
    ...
    private int findIndexOf(T item, T[] mData, int left, int right, int reason) {
        while (left < right) {
            ...
            final int cmp = compare(myItem, item);
            if (cmp < 0) {
                left = middle + 1;
            } else if (cmp == 0) {
    ...
    private int linearEqualitySearch(T item, int middle, int left, int right) {
        if (DEBUG_LOG) {
            Log.e(TAG, "linearEqualitySearch(item=" + item + ", middle=" + middle + ", left=" + left + ", right=" + right + ')');
        }
        // go left
        for (int next = middle - 1; next >= left; next--) {
            T nextItem = mData[next];
            int cmp = compare(nextItem, item);
            if (cmp != 0) {
                break;
            }
    ...
    public abstract static class SortedListAdapterCallback<T2> extends Callback<T2> {
        ...
        public void onChanged(int position, int count) {
            if (DEBUG_LOG) {
                Log.e(TAG, "onChanged(position=" + position + ", count=" + count + ')');
            }
        ...
        public void onChanged(int position, int count, Object payload) {
            if (DEBUG_LOG) {
                Log.e(TAG, "onChanged(position=" + position + ", count=" + count + ", payload=" + payload + ')');
            }
